...
class CSV_log:
    ...
    def __init__(self, pcsv, name, headdict, noise_bounds):
        self.file = "bbl"
        self.name = name
        self.headdict = headdict

        self.data = self.readcsv(StringIO(pcsv))
    ...
    def plot_all_noise(self, traces, lims): #style='fancy' gives 2d hist for response
        ...
        for i, tr in enumerate(traces):
            ...
            pc0 = plt.pcolormesh(tr.noise_gyro['throt_axis'][:-1], tr.noise_gyro['freq_axis'][:-1], tr.noise_gyro['hist2d_sm']+1.,norm=colors.LogNorm(vmin=lims[0,0],vmax=lims[0,1]),cmap=cmap)
...
class BB_log:
    def __init__(self, pheaders, pcsv, name, show, noise_bounds):
        ...
        self.heads = self.beheader(pheaders)
        self.figs = self._csv_iter(self.heads, pcsv)
    ...
    def _csv_iter(self, h, pcsv):
        figs = []
        analysed = CSV_log(pcsv, self.name, h, self.noise_bounds)
    ...
    def beheader(self, pheaders):
        lines = pheaders.split('\n')
        ### in case info is not provided by log, empty str is printed in plot
        headsdict = {'dynThrottle' :'',
                     'craftName'   :'',
                     'fwType': '',
                     'version'     :'',
                     'fwDate'        :'',
        ...
        translate_dic={'dynThrPID':'dynThrottle',
                     ...
                     'Firmware type':'fwType','firmwareType':'fwType',
        ...
        for l in lines:
            ...
            for k in translate_dic.keys():
                if k in l:
                    val =l.split(',',2)[-1]
                    headsdict.update({translate_dic[k]:val.strip('"')})
        #print(headsdict)
        return headsdict
...
def dowork():
    ...
    BB_log(pheaders,pcsv, plot_name, show, noise_bounds)
